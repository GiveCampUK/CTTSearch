        ...
        private void CreateDummyDataIfEmpty(ISession dataSession)
        {
            ...
            using (var tx = dataSession.BeginTransaction())
            {
                ...
                category = new Category
                {
                    ...
                    Tags = "Payments",
                    SearchResultLinks = new List<SearchResultLink> { new SearchResultLink { Title = "Chip & Pin Terminals", Tags = "Payments, ChipAndPin" } },
                    SubCategories = new List<Category> { new Category { Title = "Direct Debit Services", Blurb = "Direct Debit is an essential tool for charities seeking regular gifts either online or via telephone fundraising campaigns", Tags = "Payments, DirectDebit", SearchResultLinks = new List<SearchResultLink> { }, SubCategories = new List<Category> { } } }
                };
                category.SearchResultLinks.ForEach(r => dataSession.Save(r));
                category.SubCategories.ForEach(r => dataSession.Save(r));
                dataSession.Save(category);
                ...
                category = new Category
                {
                    ...
                    Tags = "Email",
                    SearchResultLinks = new List<SearchResultLink> { new SearchResultLink { Title = "Choosing An Internet Service Provider", Tags = "Email, ISP" } },
                    SubCategories = new List<Category> { 
                    new Category { Title = "Using Broadband", Blurb = "As greater numbers of people connect to broadband those left without a service are increasingly struggling to keep up.  In the voluntary and community sector large numbers of people work from home and a slow internet connection can seriously hamper productivity.", Tags = "Email, Broadband", SearchResultLinks = new List<SearchResultLink> {}, SubCategories = new List<Category> {} },
                    new Category { Title = "Building And Designing Email Newsletters", Blurb = "Different Email Clients (Outlook, Hotmail, Yahoo!, AOL, Gmail) display email slightly differently, and many of them are dependant on the web browser the user has chosen. So how do we build and design emails to fit across all systems?", Tags = "Email, Newsletters", SearchResultLinks = new List<SearchResultLink> {}, SubCategories = new List<Category> {} } }
                };
                category.SearchResultLinks.ForEach(r => dataSession.Save(r));
                category.SubCategories.ForEach(r => dataSession.Save(r));
                dataSession.Save(category);
